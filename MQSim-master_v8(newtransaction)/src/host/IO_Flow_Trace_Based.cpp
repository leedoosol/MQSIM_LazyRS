...
#if REPROGRAM
	/*
	Host_IO_Request* request_same = new Host_IO_Request;
	request_same->Arrival_time = request->Arrival_time+1;
	request_same->Enqueue_time = request->Enqueue_time + 1;
	request_same->IO_queue_info = request->IO_queue_info;
	request_same->LBA_count = request->LBA_count;
	request_same->Source_flow_id = request->Source_flow_id;
	request_same->Start_LBA = request->Start_LBA;
	request_same->Type = request->Type;
	*/
#endif
	if (request != NULL)
	{
...
#if REPROGRAM
		/*
		Submit_io_request(request_same);
