...
#ifdef REPROGRAM
	GCRpgTRQueue = new Flash_Transaction_Queue * [channel_count];
#endif
...
#ifdef REPROGRAM
		GCRpgTRQueue[channelID] = new Flash_Transaction_Queue[chip_no_per_channel];
#endif
	...
	for (std::list<NVM_Transaction_Flash *>::iterator it = transaction_receive_slots.begin(); it != transaction_receive_slots.end(); it++)
	{
		...
		case Transaction_Type::RPG:
			...
			case Transaction_Source_Type::GC_WL:
				GCRpgTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID].push_back((*it));
				break;
			default:
				PRINT_ERROR("TSU_OutOfOrder: unknown source type for a reprogram transaction!")
			}
			break;
...
{
	//std::cout<< "[DOODU_DEBUG] service_read_transaction from TSU_OutofOrder.cpp" << std::endl;
...
#if REPROGRAM
bool TSU_OutOfOrder::service_rpg_transaction(NVM::FlashMemory::Flash_Chip* chip)
{
	...
	{
		sourceQueue1 = &MappingReadTRQueue[chip->ChannelID][chip->ChipID];
		sourceQueue2 = &GCRpgTRQueue[chip->ChannelID][chip->ChipID];
		//
		//if (ftl->GC_and_WL_Unit->GC_is_in_urgent_mode(chip) && GCReadTRQueue[chip->ChannelID][chip->ChipID].size() > 0)
		//{
		//   sourceQueue2 = &GCReadTRQueue[chip->ChannelID][chip->ChipID];
	...
	{
		//std::cout << "[DOODU_DEBUG] GC_is_in_urgent_mode, Have to check this function" << std::endl;
		
		//If flash transactions related to GC are prioritzed (non-preemptive execution mode of GC), then GC queues are checked first
		if (GCReadTRQueue[chip->ChannelID][chip->ChipID].size() > 0)
		{
			sourceQueue1 = &GCReadTRQueue[chip->ChannelID][chip->ChipID];
			if (GCRpgTRQueue[chip->ChannelID][chip->ChipID].size() > 0)
			{
				sourceQueue2 = &GCRpgTRQueue[chip->ChannelID][chip->ChipID];
			}
		}
		...
		else if (GCRpgTRQueue[chip->ChannelID][chip->ChipID].size() > 0)
		{
			sourceQueue1 = &GCRpgTRQueue[chip->ChannelID][chip->ChipID];
		}
		else
		{
			return false;
	...
	issue_command_to_chip(sourceQueue1, sourceQueue2, Transaction_Type::RPG, suspensionRequired);
