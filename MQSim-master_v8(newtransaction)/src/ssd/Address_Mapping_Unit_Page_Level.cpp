...
namespace SSD_Components
{
	...
	*/
	...
	void Address_Mapping_Unit_Page_Level::allocate_page_in_plane_for_user_write(NVM_Transaction_Flash_WR* transaction, bool is_for_gc)
	{
		...
		} else { //after first access, if lpa is same, come into this part. for accurate experiment, I set Device_Level_Data_Caching_Mode TURNED_OFF(workload.xml) [doodu]
			if (is_for_gc) {
				...
				//check if an update read is required. in 'onlyW2-again.trace', only come into this 'if' part.
				if (status_intersection == prev_page_status) {
#if REPROGRAM
					/*
					Host_Components::Host_IO_Request* request_same = new Host_Components::Host_IO_Request;
					request_same->Arrival_time = CurrentTimeStamp;
					request_same->Enqueue_time = CurrentTimeStamp+1;
					request_same->IO_queue_info = 52785;
					request_same->LBA_count = 16;
					request_same->Source_flow_id = 52785;
					request_same->Start_LBA = 15074076;
					request_same->Type = Host_Components::Host_IO_Request_Type::WRITE;
					Host_Components::IO_Flow_Base::Submit_io_request(request_same);
					*/
					/*
					ftl->TSU->Prepare_for_transaction_submit();
					NVM_Transaction_Flash_WR* gc_wl_write = NULL;
					NVM_Transaction_Flash_RD* gc_wl_read = NULL;
					gc_wl_read = new NVM_Transaction_Flash_RD(Transaction_Source_Type::MAPPING, transaction->Stream_id, _my_instance->sector_no_per_page * SECTOR_SIZE_IN_BYTE,
						NO_LPA, transaction->PPA, transaction->Address, NULL, 0, NULL, 0, INVALID_TIME_STAMP);
					
					gc_wl_write = new NVM_Transaction_Flash_WR(Transaction_Source_Type::GC_WL, transaction->Stream_id, _my_instance->sector_no_per_page * SECTOR_SIZE_IN_BYTE,
						NO_LPA, NO_PPA, transaction->Address, NULL, 0, gc_wl_read, 0, INVALID_TIME_STAMP);
					...
					ftl->TSU->Submit_transaction(gc_wl_read);//Only the read transaction would be submitted. The Write transaction is submitted when the read transaction is finished and the LPA of the target page is determined
					*/
					...
					NVM_Transaction_Flash_WR* gc_wl_write = NULL;
					gc_wl_write = new NVM_Transaction_Flash_WR(Transaction_Source_Type::GC_WL, transaction->Stream_id, _my_instance->sector_no_per_page * SECTOR_SIZE_IN_BYTE,
						NO_LPA,  NO_PPA, NULL, 0, NULL, 0, INVALID_TIME_STAMP);
					gc_wl_write->ExecutionMode = WriteExecutionModeType::COPYBACK;
					ftl->TSU->Submit_transaction(gc_wl_write);
					ftl->TSU->Schedule();
					*/
					/*
					ftl->TSU->Prepare_for_transaction_submit();
					//transaction size is always page size?(SECTOR_SIZE_IN_BYTE * sector_no_per_page)
					NVM_Transaction_Flash_WR* dummy_tr = new NVM_Transaction_Flash_WR(Transaction_Source_Type::MAPPING, transaction->Stream_id, SECTOR_SIZE_IN_BYTE * sector_no_per_page,
						transaction->LPA, transaction->PPA, NULL, 0, NULL,0, CurrentTimeStamp);
					//NVM_Transaction_Flash_WR* dummy_tr = new NVM_Transaction_Flash_WR(Transaction_Source_Type::USERIO, transaction->Stream_id, SECTOR_SIZE_IN_BYTE * sector_no_per_page,
						//transaction->LPA, transaction->PPA, NULL, 0, NULL, 0, 0);
					ftl->TSU->Submit_transaction(dummy_tr);
					ftl->TSU->Schedule();
					*/


					if(transaction->W_reprogram==true) {
						/*
							In this case, [DOODU]
							1. Don't invalidate page (Don't call Invalidate_page_in_block
							2. In Allocate_block_and_page_in_plane_for_user_write(), dont increase Current_page_write_index
							I have to think about related_read when make new writeTR (NVM_Transaction_Flash_WR)
						*/
						/*
						ftl->TSU->Prepare_for_transaction_submit();
						//transaction size is always page size?(SECTOR_SIZE_IN_BYTE * sector_no_per_page)
						NVM_Transaction_Flash_WR* writeTR = new NVM_Transaction_Flash_WR(Transaction_Source_Type::USERIO, transaction->Stream_id, SECTOR_SIZE_IN_BYTE * sector_no_per_page,
							transaction->LPA, transaction->PPA, NULL, 0, NULL, (((page_status_type)0x1) << sector_no_per_page) - 1, CurrentTimeStamp);
						ftl->TSU->Submit_transaction(writeTR);
						ftl->TSU->Schedule();
						*/
						/*
						[ref : Translate_lpa_to_ppa_and_dispatch() ]
						if (transactionList.size() > 0) {
							ftl->TSU->Translate_lpa_to_ppa_and_dispatch();
							for (std::list<NVM_Transaction*>::const_iterator it = transactionList.begin();
								it != transactionList.end(); it++) {
								if (((NVM_Transaction_Flash*)(*it))->Physical_address_determined) {
									ftl->TSU->Submit_transaction(static_cast<NVM_Transaction_Flash*>(*it));
						...
						*/
						/*[ref : generate_flash_writeback_request_for_mapping_data() ]
					...
					NVM::FlashMemory::Physical_Page_Address addr;
					Convert_ppa_to_address(old_ppa, addr);
					//std::cout << "[DOODU_DEBUG] LPA(invalidated) : " << transaction->LPA << std::endl;
					block_manager->Invalidate_page_in_block(transaction->Stream_id, addr);
				} else {
