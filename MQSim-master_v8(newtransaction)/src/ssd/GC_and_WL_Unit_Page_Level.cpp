...
#ifdef NEW_SUBMIT
		/*
		만일 retention time에 다다라서 reprogram이 필요한 block이 있으면 gc_candidate_block_id로 설정
		*/
		if (free_block_pool_size < block_pool_gc_threshold) {
		flash_block_ID_type gc_candidate_block_id2 = 2040;
		PlaneBookKeepingType* pbke2 = block_manager->Get_plane_bookkeeping_entry(plane_address);
		//This should never happen, but we check it here for safty
		if (pbke2->Ongoing_erase_operations.find(gc_candidate_block_id2) != pbke2->Ongoing_erase_operations.end()) {
			std::cout << "[DOODU_DEBUG] This should never happen in GC " << std::endl;
		}
		...
		Block_Pool_Slot_Type* block2 = &pbke2->Blocks[gc_candidate_address2.BlockID];
		...
		if (block_manager->Can_execute_gc_wl(gc_candidate_address2)) {
			Stats::Total_gc_executions++;
			tsu->Prepare_for_transaction_submit();
			NVM_Transaction_Flash_ER* gc_erase_tr2 = new NVM_Transaction_Flash_ER(Transaction_Source_Type::GC_WL, pbke2->Blocks[gc_candidate_block_id2].Stream_id, gc_candidate_address2);

			//If there are some valid pages in block, then prepare flash transactions for page movement
			if (block2->Current_page_write_index - block2->Invalid_page_count > 0) {
				NVM_Transaction_Flash_RD* gc_read2 = NULL;
				NVM_Transaction_Flash_WR* gc_write2 = NULL;
				for (flash_page_ID_type pageID = 0; pageID < block2->Current_page_write_index; pageID++) {
					if (block_manager->Is_page_valid(block2, pageID)) {
						Stats::Total_page_movements_for_gc++;
						gc_candidate_address2.PageID = pageID;
						if (use_copyback) {
							gc_write2 = new NVM_Transaction_Flash_WR(Transaction_Source_Type::GC_WL, block2->Stream_id, sector_no_per_page * SECTOR_SIZE_IN_BYTE,
								NO_LPA, address_mapping_unit->Convert_address_to_ppa(gc_candidate_address2), NULL, 0, NULL, 0, INVALID_TIME_STAMP);
							gc_write2->ExecutionMode = WriteExecutionModeType::COPYBACK;
							tsu->Submit_transaction(gc_write2);
						}
						else {
							gc_read2 = new NVM_Transaction_Flash_RD(Transaction_Source_Type::GC_WL, block2->Stream_id, sector_no_per_page * SECTOR_SIZE_IN_BYTE,
								NO_LPA, address_mapping_unit->Convert_address_to_ppa(gc_candidate_address2), gc_candidate_address2, NULL, 0, NULL, 0, INVALID_TIME_STAMP);
							gc_write2 = new NVM_Transaction_Flash_WR(Transaction_Source_Type::GC_WL, block2->Stream_id, sector_no_per_page * SECTOR_SIZE_IN_BYTE,
								NO_LPA, NO_PPA, gc_candidate_address2, NULL, 0, gc_read2, 0, INVALID_TIME_STAMP);
							gc_write2->ExecutionMode = WriteExecutionModeType::SIMPLE;
							gc_write2->RelatedErase = gc_erase_tr2;
							gc_read2->RelatedWrite = gc_write2;
							tsu->Submit_transaction(gc_read2);//Only the read transaction would be submitted. The Write transaction is submitted when the read transaction is finished and the LPA of the target page is determined
						}
						gc_erase_tr2->Page_movement_activities.push_back(gc_write2);
					}
				}
			}

			block2->Erase_transaction = gc_erase_tr2;
			tsu->Submit_transaction(gc_erase_tr2);
		}
		}
next:
#endif
...
#if REPROGRAM
								NVM_Transaction_Flash_RPG* gc_rpg = NULL;
								gc_rpg = new NVM_Transaction_Flash_RPG(Transaction_Source_Type::GC_WL, block->Stream_id, sector_no_per_page * SECTOR_SIZE_IN_BYTE,
									NO_LPA, address_mapping_unit->Convert_address_to_ppa(gc_candidate_address), gc_candidate_address, NULL, 0, NULL, 0, INVALID_TIME_STAMP);
								tsu->Submit_transaction(gc_rpg);
								
								/*
								gc_read2 = new NVM_Transaction_Flash_RD(Transaction_Source_Type::GC_WL, block->Stream_id, sector_no_per_page * SECTOR_SIZE_IN_BYTE,
									NO_LPA, address_mapping_unit->Convert_address_to_ppa(gc_candidate_address), gc_candidate_address, NULL, 0, NULL, 0, INVALID_TIME_STAMP, true);
								*/
								 //inconsistency found in GC/WL? Inconsistency found when moving a page for GC/WL
