...
void Input_Stream_Manager_NVMe::segment_user_request(User_Request *user_request)
{
	...
	unsigned int req_size = user_request->SizeInSectors; //same to trace's file's size in sector(forth column) [doodu]
	...
	while (handled_sectors_count < req_size)
	{
		...
		LPA_type lpa = internal_lsa / host_interface->sectors_per_page; // always same if access to same trace's third column [doodu]
