...
NVM::FlashMemory::Physical_Page_Address* page_address_debug;
				...
				for (unsigned int dieID = 0; dieID < die_no_per_chip; dieID++) {
					...
					for (unsigned int planeID = 0; planeID < plane_no_per_die; planeID++) {
						...
						for (unsigned int stream_cntr = 0; stream_cntr < total_concurrent_streams_no; stream_cntr++) {
							//std::cout << "call Get_a_free_block()" << std::endl;
#if REPROGRAM
							NVM::FlashMemory::Physical_Page_Address* page_address_dummy = NULL;
							plane_manager[channelID][chipID][dieID][planeID].Data_wf[stream_cntr] = plane_manager[channelID][chipID][dieID][planeID].Get_a_free_block(stream_cntr, false, page_address_dummy);
							plane_manager[channelID][chipID][dieID][planeID].Translation_wf[stream_cntr] = plane_manager[channelID][chipID][dieID][planeID].Get_a_free_block(stream_cntr, true, page_address_dummy);
							plane_manager[channelID][chipID][dieID][planeID].GC_wf[stream_cntr] = plane_manager[channelID][chipID][dieID][planeID].Get_a_free_block(stream_cntr, false, page_address_dummy);
#elif BASELINE
...
#if REPROGRAM
	Block_Pool_Slot_Type* PlaneBookKeepingType::Get_a_free_block(stream_id_type stream_id, bool for_mapping_data, NVM::FlashMemory::Physical_Page_Address* page_address)
	{
		...
		//doodu
		//std::cout << "[DOODU_DEBUG] [Get_a_free_block] Get_free_block_pool_size(): " << Get_free_block_pool_size() << std::endl;
#if REPROGRAM
		/*
		The problem is, BlockID can be same though plane, chip, channel are different [doodu]
		MQSim keeps 'Block_usage_history' per plane already
		*/
		NVM::FlashMemory::Physical_Page_Address *page_address_tmp;
	

		//page_address_tmp = page_address;
		/* If NULL, it comes from composition of SSD_Components(constructor of class SSD_Components) [doodu] */
		if (page_address != NULL) {
			
			Blocks_Turbo.push_back(new_block);
			struct Blk_meta blk_meta;
			blk_meta.ChannelID = page_address->ChannelID;
			blk_meta.ChipID = page_address->ChipID;
			blk_meta.DieID = page_address->DieID;
			blk_meta.PlaneID = page_address->PlaneID;
			blk_meta.BlockID = new_block->BlockID;
			Blocks_Turbo_M.push_back(blk_meta);

			/* page_address can be deleted by NVM_PHY_ONFI::broadcastTransactionServicedSignal(). so I don't use page_address */
			//Blocks_Turbo_meta.push_back(page_address);
		}
	
		int size = Blocks_Turbo.size();
		int size_m = Blocks_Turbo_M.size();
		/*
		std::cout << "size: "<<size << std::endl;
		for (int i = 0; i < size; i++) {
			if (Blocks_Turbo[i] != NULL) {
				std::cout << " BlockID (new):" << Blocks_Turbo[i]->BlockID << std::endl;
				std::cout << " ChannelID:" << Blocks_Turbo_M[i].ChannelID << ", Chip:" << Blocks_Turbo_M[i].ChipID << ", Die: " << Blocks_Turbo_M[i].DieID << ", Plane:" << Blocks_Turbo_M[i].PlaneID << ", BlockID(new): " << Blocks_Turbo_M[i].BlockID << std::endl;
			}
		}
		*/
		/* new_block's BlockID is already assigned */
		/*
		if (page_address != NULL) {
			//std::cout << "BlockID(prev): " << page_address_tmp->BlockID << ", BlockID(new):" << new_block->BlockID << ", page_write_index(new):" << new_block->Current_page_write_index<<std::endl;
			//std::cout << " ChannelID:" << page_address_tmp->ChannelID << ", Chip:" << page_address_tmp->ChipID << ", Die: " << page_address_tmp->DieID << ", Plane:" << page_address_tmp->PlaneID << ",BlockID(prev): " <<page_address_tmp->BlockID<<",BlockID(new):"<< new_block->BlockID << std::endl;
		}
		*/	
