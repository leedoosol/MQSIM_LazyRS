...
namespace NVM
{
	namespace FlashMemory
	{
		...
		void Flash_Chip::start_command_execution(Flash_Command* command)
		{
			...
			targetDie->Expected_finish_time = Simulator->Time() + Get_command_execution_latency(command->CommandCode, command->Address[0].PageID, 0);
		...
		void Flash_Chip::finish_command_execution(Flash_Command* command)
		{
			...
			targetDie->STAT_TotalReadTime += Get_command_execution_latency(command->CommandCode, command->Address[0].PageID,0);
...
#if REPROGRAM
				case CMD_RPG:
					DEBUG("Channel " << this->ChannelID << " Chip " << this->ChipID << "- Finished executing read (reprogram) command")
