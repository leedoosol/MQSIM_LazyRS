...
#if REPROGRAM
			sim_time_type Get_command_execution_latency(command_code_type CMDCode, flash_page_ID_type pageID, bool isReprogam)
#elif BASELINE
...
#endif
			{
				...
				{	
#if REPROGRAM
					case CMD_RPG: {
						return _readLatency[latencyType] + _RBSignalDelayRead + 2*_programLatency[latencyType] + _RBSignalDelayWrite;
					}
					...
					case CMD_READ_PAGE_COPYBACK_MULTIPLANE: {
						//std::cout << "[DOODU_DEBUG] readLatency: "<<_readLatency[latencyType] << std::endl; <- follow the ssdconfig.xml file set
						//for example, in MLC set, _readLatency[anything] = 75000 if Page_Read_Latency_MSB == 75000 in ssdconfig.xml file.
